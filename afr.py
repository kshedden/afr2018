...
for group in "oral", "nasal":
    for outcome in "bucketacc", "bucketcomp":
        ...
        mean = np.concatenate((result1.fe_mean, result1.vcp_mean,
                               result1.vc_mean))
        ...
        if use_vb:
            result2 = model2.fit_vb(
                result1.params, np.ones_like(result1.params), verbose=True)
            out.write("\n\n")
            out.write(result2.summary().as_text())
            out.write("\n\n")
            mean = np.concatenate((result2.fe_mean, result2.vcp_mean,
                                   result2.vc_mean))
